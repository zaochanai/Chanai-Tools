...
def _snap_nodes_to_prev_fkik_key_translation(nodes=None, frame=None):
    ...
    Runs with undo+autokey suspended.
    """
    if frame is None:
        frame = cmds.currentTime(query=True)
    if nodes is None:
    ...
    def _do():
        ...
        return bool(_key_nodes_translate_from_time(nodes, src_time=prev_t, dst_time=frame))

    return bool(_with_undo_and_autokey_suspended(_do))
...
class BipWindow(QtWidgets.QDialog):
    ...
    def _on_step_frame(self):
        ...
        try:
            ...
            # Key aligned translate on this tread frame immediately as part of the
            # user's action (single undo step), then the background snap can stay
            # undo-suspended.
            try:
                frame = cmds.currentTime(query=True)
            except Exception:
            ...
            if frame is not None:
                ...
                try:
                    keyed = align_nodes_on_frame_to_prev_fkik_key(sel, frame=frame)
                except Exception:
                    keyed = 0
                _diag_snapshot("tread_click_after_align", frame=frame, nodes=sel, extra={"keyed": keyed})
        finally:
            cmds.undoInfo(closeChunk=True)
        add_step_mark()
        if self.enable_marks.isChecked():
    ...
    def _rebuild_xform_watch(self):
        ...
        if not sel:
            return
        # Watch translate changes on selected nodes.
        # Intentionally do NOT watch rotate: tread enforcement should only keep
        # translation aligned; rotation edits should remain free.
        for n in sel:
            for axis in ("X", "Y", "Z"):
                plug = "{}.translate{}".format(n, axis)
                try:
    ...
    def _on_xform_changed(self, *_):
        # 立即对齐（不等待拖拽结束）
        self._enforce_step_snap_immediate()
    ...
    def _enforce_step_snap_deferred(self):
        ...
        if self._step_enforce_guard:
            return
        # Prefer a timer debounce to avoid snapping every attributeChange.
        if self._step_snap_timer is not None:
            try:
                self._step_snap_timer.stop()
            except Exception:
                pass
            try:
                self._step_snap_timer.start()
                return
            except Exception:
                pass

        # Fallback: deferred execution.
        if self._step_enforce_scheduled:
            return
        self._step_enforce_scheduled = True
        try:
            maya_utils.executeDeferred(self._enforce_step_snap_now)
        except Exception:
            cmds.evalDeferred(self._enforce_step_snap_now)

    def _enforce_step_snap_immediate(self):
        """立即对齐踩踏帧（不等待拖拽结束，不污染撤销）"""
        # Respect UI toggle
        if not self.enable_marks.isChecked():
            return
        # 尊重撤销/重做的抑制期
        ...
        try:
            _diag_snapshot("tread_snap_immediate_before", frame=frame, nodes=sel)
            _snap_nodes_to_prev_fkik_key_translation(nodes=sel, frame=frame)
            try:
                cmds.refresh(force=True)
            except Exception:
                pass
            _diag_snapshot("tread_snap_immediate_after", frame=frame, nodes=sel)
        finally:
            self._step_enforce_guard = False
    ...
    def _enforce_step_snap_now(self):
        ...
        if not is_step:
            # If the user edits a slide key pose, downstream tread frames
            # should follow automatically.
            self._step_enforce_guard = True
            try:
                _diag_snapshot("tread_propagate_before", frame=frame, nodes=sel)
                aligned = align_future_tread_frames_to_prev_fkik_key(sel, start_frame=frame)
                _diag_snapshot("tread_propagate_after", frame=frame, nodes=sel, extra={"aligned": aligned})
            finally:
                self._step_enforce_guard = False
            return
        ...
        try:
            ...
            try:
                ...
                pass
            _diag_snapshot("tread_snap_deferred_after", frame=frame, nodes=sel)
        finally:
            self._step_enforce_guard = False
    ...
    def _on_selection_changed(self):
        ...
        if self.enable_marks.isChecked():
            ...
            try:
                ...
                pass
        if self.enable_marks.isChecked():
            # Force refresh so switching controllers/characters updates marks.
